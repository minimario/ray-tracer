...
let tests = "Test Suite for Reflections" >::: [
    ...
    (fun _ ->
    let s = sphere in 
    let m = {defaultMaterial with ambient = 1.} in
    ...
    assert (equalColor result (color 1.9 1.9 1.9))
    );
    ...
    let light = point_light (point 0. 0. (-10.)) (color 1. 1. 1.) in
    let result = lighting m light position eyev normalv in
    assert (equalColor result (color 1. 1. 1.))
    );
    ...
    let result = lighting m light position eyev normalv in
    assert (equalColor result (color 0.7364 0.7364 0.7364))
    );
    ...
    let light = point_light (point 0. 10. (-10.)) (color 1. 1. 1.) in
    let result = lighting m light position eyev normalv in
    assert (equalColor result (color 1.6364 1.6364 1.6364))
    );
    ...
    let light = point_light (point 0. 0. 10.) (color 1. 1. 1.) in
    let result = lighting m light position eyev normalv in
    assert (equalColor result (color 0.1 0.1 0.1))
    );
